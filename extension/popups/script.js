...
document.getElementById("server-url").defaultValue =
    "https://coscratchapi.replit.app";
...
}
{
    (async () => {
        ...
        document.getElementById("server-url").value =
            serverUrlValue || "https://coscratchapi.replit.app";
    })();
}
...
document.getElementById("switch-theme")?.addEventListener("click", function () {
    chrome.storage.local.get("theme", (data) => {
        ...
        document
            .querySelector("body")
            .classList.replace(`${theme}-mode`, `${newTheme}-mode`);
    });
});

let settingsDropdown = false;
document.getElementById("settings").style.display = settingsDropdown
    ? "flex"
    : "none";
document
...
chrome.runtime.sendMessage({ meta: "getUsernamePlus" }, function (info) {
    ...
    document
        .querySelector("input#custom-server")
        ?.addEventListener("change", function () {
            ...
            if (serverUrlField.value !== "https://coscratchapi.replit.app") {
    ...
    document
        .querySelector("input#server-url")
        ?.addEventListener("change", function () {
            ...
            if (!value) {
                document.querySelector("input#server-url").value =
                    "https://coscratchapi.replit.app";
                chrome.storage.local.set({
                    "server-url": "https://coscratchapi.replit.app",
                });
            } else {
    ...
    setTimeout(() => {
        chrome.runtime.sendMessage({ meta: "getUsernamePlus" }, setSignedin);
    }, 1000);
    ...
    function addFriendGUI(name) {
        if (name?.toLowerCase() in alreadyAdded) {
            return;
        }
    ...
    async function addFriend(name) {
        ...
        let response = await fetch(`${apiUrl}/friends/${username}/${name}`, {
            method: "POST",
    ...
    function removeFriend(name) {
        ...
        fetch(`${apiUrl}/friends/${username}/${name}`, {
            method: "DELETE",
            headers: { authorization: token },
        });
    }
    ...
    {
        (async () => {
            document.querySelector("#privme").checked = await (
                await fetch(`${apiUrl}/privateMe/${username}`, {
                    headers: { authorization: token },
                })
            ).json();
        })();
    ...
    document.querySelector("#privme")?.addEventListener("change", (event) => {
        ...
        fetch(`${apiUrl}/privateMe/${username}/${on}`, {
            method: "put",
            headers: { authorization: token },
        });
    });
...
(async () => {
    document.querySelector("#notifs").checked =
        (await chrome.storage.local.get(["notifs"]))?.notifs ?? false;
})();
...
(async () => {
    document.querySelector("#ping-sounds").checked =
        (await chrome.storage.local.get(["ping"]))?.ping ?? false;
    document.querySelector("#badges").checked = !(
        (await chrome.storage.local.get(["badges"]))?.badges ?? false
    );
})();
